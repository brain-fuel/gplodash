...
// Adapted from samber/lo v1.53.0 it/math.go (MIT), authored in Go+.
package it
...
func RangeFrom[T lo.Integer | lo.Float](start T, elementNum int) iter.Seq[T] {
	step := lo.Ternary(elementNum < 0, -1, 1)
	length := elementNum * step
...
func RangeWithSteps[T lo.Integer | lo.Float](start, end, step T) iter.Seq[T] {
	return func(yield func(T) bool) {
		if start == end || step == 0 {
			return
		}
...
func Sum[T lo.Float | lo.Integer | lo.Complex](collection iter.Seq[T]) T {
...
func SumBy[T any, R lo.Float | lo.Integer | lo.Complex](collection iter.Seq[T], transform func(item T) R) R {
	var sum R
...
func Product[T lo.Float | lo.Integer | lo.Complex](collection iter.Seq[T]) T {
...
func ProductBy[T any, R lo.Float | lo.Integer | lo.Complex](collection iter.Seq[T], transform func(item T) R) R {
	var product R = 1
	for item := range collection {
...
func Mean[T lo.Float | lo.Integer](collection iter.Seq[T]) T {
...
func MeanBy[T any, R lo.Float | lo.Integer](collection iter.Seq[T], transform func(item T) R) R {
	var sum R
	var length R
	for item := range collection {
		sum += transform(item)
...
func Mode[T lo.Integer | lo.Float](collection iter.Seq[T]) []T {
	mode := make([]T, 0)
	maxFreq := 0
