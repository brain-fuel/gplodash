...
// Adapted from samber/lo v1.53.0 it/find.go (MIT), authored in Go+.
package it
...
func Min[T lo.Ordered](collection iter.Seq[T]) T {
...
func MinIndex[T lo.Ordered](collection iter.Seq[T]) (T, int) {
...
func Max[T lo.Ordered](collection iter.Seq[T]) T {
...
func MaxIndex[T lo.Ordered](collection iter.Seq[T]) (T, int) {
...
func Nth[T any, N lo.Integer](collection iter.Seq[T], nth N) (T, error) {
	value, ok := seqNth(collection, nth)

	return value, lo.Validate(ok, "nth: %d out of bounds", nth)
}

func seqNth[T any, N lo.Integer](collection iter.Seq[T], nth N) (T, bool) {
	if nth >= 0 {
		var i N
		for item := range collection {
			if i == nth {
...
func NthOr[T any, N lo.Integer](collection iter.Seq[T], nth N, fallback T) T {
	value, ok := seqNth(collection, nth)
	if !ok {
		return fallback
	}
...
func NthOrEmpty[T any, N lo.Integer](collection iter.Seq[T], nth N) T {
