...
// Adapted from samber/lo v1.53.0 it/seq.go (MIT), authored in Go+.
package it
...
import (
	...
	"goforge.dev/gplodash"
	"goforge.dev/gplodash/mutable"

	gpiter "goforge.dev/goplus/std/iter"
)
...
func Filter[T any, I ~func(func(T) bool)](collection I, predicate func(item T) bool) I {
	return I(gpiter.Of(iter.Seq[T](collection)).Filter(predicate).Seq())
}
...
func Map[T, R any](collection iter.Seq[T], transform func(item T) R) iter.Seq[R] {
	// Facade over std/iter: the general lazy engine and its laws live there.
	return gpiter.Map(gpiter.Of(collection), transform).Seq()
}
...
func FilterMap[T, R any](collection iter.Seq[T], callback func(item T) (R, bool)) iter.Seq[R] {
	return gpiter.FilterMap(gpiter.Of(collection), callback).Seq()
}
...
func FlatMap[T, R any](collection iter.Seq[T], transform func(item T) iter.Seq[R]) iter.Seq[R] {
	return gpiter.FlatMap(gpiter.Of(collection), func(item T) gpiter.Seq[R] {
		return gpiter.Of(transform(item))
	}).Seq()
}
...
func Reduce[T, R any](collection iter.Seq[T], accumulator func(agg R, item T) R, initial R) R {
	return gpiter.Fold(gpiter.Of(collection), initial, accumulator)
}
...
func Uniq[T comparable, I ~func(func(T) bool)](collection I) I {
	return I(gpiter.Uniq(gpiter.Of(iter.Seq[T](collection))).Seq())
}
...
func UniqBy[T any, U comparable, I ~func(func(T) bool)](collection I, transform func(item T) U) I {
	return I(gpiter.UniqBy(gpiter.Of(iter.Seq[T](collection)), transform).Seq())
}
...
func Chunk[T any](collection iter.Seq[T], size int) iter.Seq[[]T] {
	...
	return gpiter.Chunk(gpiter.Of(collection), size).Seq()
}
...
func Reverse[T any, I ~func(func(T) bool)](collection I) I {
	return I(gpiter.Of(iter.Seq[T](collection)).Reverse().Seq())
}
...
func Drop[T any, I ~func(func(T) bool)](collection I, n int) I {
	...
	return I(gpiter.Of(iter.Seq[T](collection)).Drop(n).Seq())
}
...
func DropWhile[T any, I ~func(func(T) bool)](collection I, predicate func(item T) bool) I {
	return I(gpiter.Of(iter.Seq[T](collection)).DropWhile(predicate).Seq())
}
...
func Take[T any, I ~func(func(T) bool)](collection I, n int) I {
	...
	return I(gpiter.Of(iter.Seq[T](collection)).Take(n).Seq())
}
...
func TakeWhile[T any, I ~func(func(T) bool)](collection I, predicate func(item T) bool) I {
	return I(gpiter.Of(iter.Seq[T](collection)).TakeWhile(predicate).Seq())
}
...
// Will iterate through the entire sequence.
// Play: https://go.dev/play/p/9jthUzgF-u
func IsSorted[T lo.Ordered](collection iter.Seq[T]) bool {
...
// Will iterate through the entire sequence.
// Play: https://go.dev/play/p/0XrQKOk-vw
func IsSortedBy[T any, K lo.Ordered](collection iter.Seq[T], transform func(item T) K) bool {
	first := true
	var prev K
	for item := range collection {
		key := transform(item)
